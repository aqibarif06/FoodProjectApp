...
} = require("../services/aiReviewAnalyzer");
...
exports.generateFoodAI = catchAsync(async (req, res) => {
  ...
  if (!name || !category || !spiceLevel || !price) {
    return res.status(400).json({
      success: false,
      message:
        "name, category, spiceLevel and price are required",
    });
  }

  const aiData =
    await aiService.generateDishDescription({
      name,
      category,
      spiceLevel,
      price,
...
// ======================================================
// GENERATE AI DATA AND SAVE
// ======================================================

exports.generateAndSaveFoodAI = catchAsync(
  async (req, res) => {
    ...
    if (!aiData) {
      return res.status(500).json({
        success: false,
        message:
          "AI failed to generate metadata",
      });
    }
    ...
    res.status(200).json({
      success: true,
      message:
        "AI metadata generated and saved",
      data: aiData,
    });
  }
);
...
exports.analyzeRestaurantReviews =
